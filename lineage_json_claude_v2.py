...
Required columns (case-insensitive):
    level                   Integer lineage depth — drives LEVEL_<n> key naming in output
...
Each file contains a fully recursive nested lineage tree.
The LEVEL_<n> nesting key is named after the actual `level` value of the
ancestor rows, so same-level and cross-level dependencies are captured exactly.
Example (root row has level=2; its parent row has level=3):
    {
...
If a node has parents at multiple distinct levels, it will contain
multiple LEVEL_<x> keys (one per distinct parent level).
...
def build_lineage_node(
    ...
    ``visited`` tracks the current DFS path (immutably updated) for cycle detection.

    LEVEL_<n> key naming uses the **actual level value of ancestor rows**, not an
    incrementing depth counter.  This means:
      - A parent row at level 3 always appears under key "LEVEL_3", regardless of
        how deep in the recursion it is found.
      - If a node has parents at multiple distinct levels (e.g., 3 and 5), it will
        contain both "LEVEL_3" and "LEVEL_5" keys — one list per distinct parent level.
      - Same-level dependencies (e.g., level 3 → level 3) are represented correctly
        as "LEVEL_3" nested inside a level-3 node.
    """
    node: dict = {
        "level": row.level,
    ...
    # Recurse into parents of this row's source node
    ...
    if not parent_rows:
        return node  # No further ancestry – omit LEVEL_<n> keys

    visited = visited | {parent_key}  # immutable update — siblings don't share state

    # Group parents by their actual level value so each gets a correctly named key
    level_groups: Dict[int, List[LineageRow]] = defaultdict(list)
    for parent_row in parent_rows:
        level_groups[parent_row.level].append(parent_row)

    for lvl in sorted(level_groups.keys()):
        level_key = f"LEVEL_{lvl}"
        child_nodes = [
            build_lineage_node(pr, graph, visited)
            for pr in level_groups[lvl]
        ]
        node[level_key] = child_nodes
...
def write_lineage_json(
    ...
    LEVEL_<n> key names are driven by the actual level values in the data.
    """
    ...
    for row in rows:
        node = build_lineage_node(row, graph, visited_root)
