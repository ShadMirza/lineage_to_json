...
def level_key_for_row(row_level: Optional[int]) -> str:
    """
    Return recursive key name for the next upstream level.
    If row_level is unavailable, use LEVEL_NEXT.
    """
    if row_level is None:
        return "LEVEL_NEXT"
    return f"LEVEL_{row_level + 1}"
